...
| 34 | Sharpness: previews now render through PowerPoint (`render_preview.py`, AppleScript PDF export, 160 dpi) with the full Libre Franklin family installed from the Google Fonts source; the round forms got a label rule (`_ring_label`: side labels clear the node and centre on it, top and bottom labels sit above or below, boxes clamped to the page), a track ring, 2pt arrows, a tinted lens on the zoom, white separators on rings and venn, blue-tint connectors | engine + tooling | ✅ SHIPPED 2026-09-25 |
