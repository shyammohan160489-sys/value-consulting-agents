...
## Round 11 · the framework layer and icons (25 Sep 2026)

Shyam, on the enablement decks: "major issues with how some of the slides are using lots of boxes and tiles... pick up nice frameworks from the BCG examples: a flywheel, a cascading framework, a funnel, a deep dive like a zoom... use icons." Diagnosis: the engine's recipes were measured from two decks that are mostly tables and cards, so tables and cards were what it reached for; the framework forms were never recipes and there was no icon library. Both built as engine v5.0. Icons: the Lucide set (2,118, ISC) fetched into `knowledge/design-system/icons/lucide/` and drawn as native freeform strokes from the SVG paths (`icon_glyph`, `lucide_search`), so they stay crisp, recolourable and editable in PowerPoint and Google Slides with no image files and no font. Forms, sampled from the archive's grammar (icon-led point rows, temple pillars, funnels with values, layered stacks, hypothesis circles, venn overlaps): flywheel, cascade, funnel, zoom, value_map, chevron_flow, rings, stack (bands or ziggurat), hub_spoke, icon_rows, venn, pillars. Catalog T71–T83. Rule in SKILL.md: the claim's shape picks the form; tiles are the last resort and never twice in a row; one icon per concept on structural pages. Sampler `example_apex_frameworks_build.py`, twelve pages, QA'd in three render passes (label collisions on the round forms, wrapped chevron labels, the value map's rhythm, the funnel's label column). The save-time lint caught an "X, not Y" tail in my own footnote on the first pass.
